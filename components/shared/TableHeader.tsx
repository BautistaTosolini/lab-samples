...
const TableHeader = ({ print = false, type }: TableHeaderProps) => {
  return (
    <div className={`flex gap-5 bg-white rounded-lg p-4 border border-gray-300 ${print ? 'text-sm' : ''} ${type === 'staining' ? 'w-[1020px]' : ''}`}>
      ...
      </div>
      {
        ...
        :
          <div className='hidden sm:flex flex-col'>
            <h2 className={`text-lg font-semibold mb-4 text-center border-b-2 border-black w-40 ${print ? 'text-sm h-8' : ''}`}>Estado</h2>
            <div className='grid grid-cols-4 gap-2 text-center'>
            <span className={`w-40 font-semibold border-b-2 border-black ${print ? 'h-8' : ''}`}>Tinción Realizada</span>
            </div>
          </div>
      }
    </div>
